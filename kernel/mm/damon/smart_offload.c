...
/*
 * DAMON-based offloading PTE scanning to smart device
 *
 * Author: SeongJae Park <sj@kernel.org>
 */

#define pr_fmt(fmt) "damon-smart-offload: " fmt
...
static struct damos_quota damon_smart_offload_quota = {
...
DEFINE_DAMON_MODULES_DAMOS_QUOTAS(damon_smart_offload_quota);
...
static struct damos_watermarks damon_smart_offload_wmarks = {
...
DEFINE_DAMON_MODULES_WMARKS_PARAMS(damon_smart_offload_wmarks);

static struct damon_attrs damon_smart_offload_mon_attrs = {
...
static struct damos_stat damon_smart_offload_stat;
DEFINE_DAMON_MODULES_DAMOS_STATS_PARAMS(damon_smart_offload_stat,
		reclaim_tried_regions, reclaimed_regions, quota_exceeds);

static struct damon_ctx *ctx;
static struct damon_target *target;

static struct damos *damon_smart_offload_new_scheme(void)
{
	struct damos_access_pattern pattern = {
		...
		.min_age_region = min_age /
			damon_smart_offload_mon_attrs.aggr_interval,
	...
	return damon_new_scheme(
			...
			0,
			/* under the quota. */
			&damon_smart_offload_quota,
			/* (De)activate this according to the watermarks. */
			&damon_smart_offload_wmarks,
...
static int damon_smart_offload_apply_parameters(void)
{
	...
	err = damon_set_attrs(ctx, &damon_smart_offload_mon_attrs);
	if (err)
		goto out;

	err = -ENOMEM;
	scheme = damon_smart_offload_new_scheme();
	if (!scheme)
		goto out;
...
static int damon_smart_offload_turn(bool on)
{
	int err;
	...
	err = damon_smart_offload_apply_parameters();
	if (err)
		return err;
...
static int damon_smart_offload_enabled_store(const char *val,
		const struct kernel_param *kp)
{
	...
	err = damon_smart_offload_turn(enable);
	if (err)
		return err;
...
static const struct kernel_param_ops enabled_param_ops = {
	.set = damon_smart_offload_enabled_store,
	.get = param_get_bool,
};
...
static int damon_smart_offload_after_aggregation(struct damon_ctx *c)
{
	struct damos *s;

	/* update the stats parameter */
	damon_for_each_scheme(s, c)
		damon_smart_offload_stat = s->stat;

	return damon_smart_offload_handle_commit_inputs();
}

static int damon_smart_offload_after_wmarks_check(struct damon_ctx *c)
{
	return damon_smart_offload_handle_commit_inputs();
}
...
static int __init damon_smart_offload_init(void)
{
	...
	ctx->callback.after_wmarks_check = damon_smart_offload_after_wmarks_check;
	ctx->callback.after_aggregation = damon_smart_offload_after_aggregation;

	/* 'enabled' has set before this function, probably via command line */
	if (enabled)
		err = damon_smart_offload_turn(true);
