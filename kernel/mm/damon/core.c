 ...
 * Return: 0 on success, negative error code otherwise.
 */
static int __damon_start(struct damon_ctx *ctx, int node)
{
	int err = -EBUSY;
	...
	if (!ctx->kdamond) {
		...
		ctx->kdamond = kthread_create_on_node(kdamond_fn, ctx, node, "kdamond.%d",
				nr_running_ctxs);
 ...
 * Return: 0 on success, negative error code otherwise.
 */
int damon_start_on_node(struct damon_ctx **ctxs, int nr_ctxs, bool exclusive, int node)
{
	int i;
	int err = 0;
